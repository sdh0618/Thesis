...
\\
Finally, we study the spatial morphology of the 511 keV signal toward the Galactic center using data from \textit{INTEGRAL/SPI}. We confirm that the 511 keV signal also traces the old stellar population in the Galactic bulge, which is similar to the \textit{Fermi} GeV excess. Using a 3D smoothing kernel, we find that the signal is smeared out over a characteristic length scale of 150 $\pm$ 50 pc. We show that positron propagation prior to annihilation can explain the overall phenomenology of the 511 keV signal.
}
